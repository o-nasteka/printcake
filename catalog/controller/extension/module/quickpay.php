...
class ControllerExtensionModuleQuickpay extends Controller {
	public function index() {
			...
			$data['text_cheapering_product'] = $this->language->get('text_cheapering_product');
			
			$data['attribute_groups'] = $this->model_catalog_product->getProductAttributes($product_id);
			$data['review_status'] = $this->config->get('config_review_status');
			
			if ($this->config->get('module_quickpay_status_your_name')) {$data['module_quickpay_status_your_name'] = $this->config->get('module_quickpay_status_your_name');} else {$data['module_quickpay_status_your_name'] = false;}
			if ($this->config->get('module_quickpay_status_phone')) {$data['module_quickpay_status_phone'] = $this->config->get('module_quickpay_status_phone');} else {$data['module_quickpay_status_phone'] = false;}
			if ($this->config->get('module_quickpay_status_email')) {$data['module_quickpay_status_email'] = $this->config->get('module_quickpay_status_email');} else {$data['module_quickpay_status_email'] = false;}
			if ($this->config->get('module_quickpay_status_comment')) {$data['module_quickpay_status_comment'] = $this->config->get('module_quickpay_status_comment');} else {$data['module_quickpay_status_comment'] = false;}
			if ($this->config->get('module_quickpay_format')) {$data['module_quickpay_format'] = $this->config->get('module_quickpay_format');} else {$data['module_quickpay_format'] = false;}
			
			if ($this->config->get('module_quickpay_text')) {
				$text = $this->config->get('module_quickpay_text');
				$data['module_quickpay_text'] = html_entity_decode($text[$this->session->data['language']], ENT_QUOTES, 'UTF-8');
			} else {
				$data['module_quickpay_text'] = false;
			}
			...
			if ($product_info['image']) {
				$data['image'] = $this->model_tool_image->resize($product_info['image'], 500, 500);
			} else {
				$data['image'] = $this->model_tool_image->resize('no_image.png', 500, 500);
			}
			...
			if ($imgs) {
				foreach ($imgs as $imgi) {
					$data['imgt'][] = array(
						'thumb' => $this->model_tool_image->resize($imgi['image'], 500, 500)
					);
				}
			}
					...
					foreach ($option['product_option_value'] as $option_value) {
						if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
							...
							if ($option_value['image'] and $option_value['image'] != "no_image.jpg") {
								...
								$image_popup = $this->model_tool_image->resize($option_value['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_product_width')*1.5, $this->config->get('theme_' . $this->config->get('config_theme') . '_image_product_height')*1.5);
							} else {
								$image = false;
								$image_popup = false;
							}
	...
	public function quick() {
		...
		if ($this->config->get('module_quickpay_text')) {
			$text = $this->config->get('quickpay_text');
			$data['quickpay_text'] = html_entity_decode($text[$this->session->data['language']], ENT_QUOTES, 'UTF-8');
		} else {
			$data['quickpay_text'] = false;
		}
		if (!isset($this->request->post['zachita']) &&  ($this->config->get('quickpay_format') == "checkbox" && $data['quickpay_text'])) {
			$json['error']['zachita'] = $this->language->get('error_zachita');
		}
		
		if (!$this->config->get('module_quickpay_status_your_name') and !$this->config->get('module_quickpay_status_phone') and !$this->config->get('module_quickpay_status_email')) {
			$json['error'] = 'empty_setting';
			$json['empty'] = $this->language->get('error_empty_quickpay');
		}
