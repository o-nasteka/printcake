...
class ControllerExtensionModuleNewstemplate extends Controller {
	public function index($setting) {
		...
		if ($this->config->get('module_newstemplate_limit_h1')) {$limit_h1 = $this->config->get('module_newstemplate_limit_h1');} else {$limit_h1 = '60';}
		if ($this->config->get('module_newstemplate_limit_description')) {$limit_description = $this->config->get('module_newstemplate_limit_description');} else {$limit_description = '60';}
		if ($this->config->get('module_newstemplate_image_width')) {$width = $this->config->get('module_newstemplate_image_width');} else {$width = '100';}
		if ($this->config->get('module_newstemplate_image_height')) {$height = $this->config->get('module_newstemplate_image_height');} else {$height = '100';}
